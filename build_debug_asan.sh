...
for source_file in *.c; do
	...
	if ! clang-17 -c -std=c99 -Weverything \
		...
		-DDEBUG -fsanitize=address -fno-omit-frame-pointer -fno-optimize-sibling-calls -O1 -g "$source_file" -o "$o_file"; then
		exit 1
	fi
done

if ! clang -fsanitize=address ./*"$obj_suffix" -o app.exe; then
	exit 1
fi
