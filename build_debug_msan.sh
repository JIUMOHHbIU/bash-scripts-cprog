...
clang_compile_args_debug_msan=("-g" "-DDEBUG" "-fsanitize=memory" "-fno-omit-frame-pointer" "-fno-optimize-sibling-calls" "-fPIE")
...
if ! clang-17 "${clang_compile_args_default[@]}" "${clang_compile_args_debug_msan[@]}" ./*.c; then
    exit 1
fi
